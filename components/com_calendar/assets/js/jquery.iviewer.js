...
$.widget( "ui.iviewer", $.ui.mouse, {
    ...
    options : {
        ...
        **/
		...
		isRotated_element: "#rotation_used",
		left: 100,
		top: 0,
		img_width: 0,
		img_height: 0,
...
//		var left_element = this.options.left_element;
//		var top_element = this.options.top_element;
//		var width_element =  this.options.width_element;
//		var height_element =  this.options.height_element;
		
//		console.log(jQuery(left_element).val());
    ...
    loadImage: function( src )
    {
        this.current_zoom = this.options.zoom;
        var me = this;
		//var loader = "<div class='preload-cover'><img class='preloader' src='https://vlastnykalendar.sk/components/com_calendar/assets/images/preloader.gif' alt='Loading'/><br /></small>Čakám na obrázok ..</small></div>";
		var loader = ''
    ...
    _imageLoaded: function(src) {
		...
		this.container.find(".preload-cover").remove();
        this.container.addClass("iviewer_cursor");

		
		
      if(this.options.zoom == "fit"){
		  if(this.options.img_width == "auto" || this.options.img_height == "auto") {
            this.fit(true);
		  }
		  else {
			  var current_load;
			  this.current_load = "load";
			  this.loadPos(this.options.img_width,this.options.img_height);
		  }
      }
      else if(this.options.img_width == "auto" || this.options.img_height == "auto") {
    ...
    /**
    * load image in the container
    *
    **/
    loadPos: function(width, height)
    {
        ...
        if(choose_left){
            new_zoom = this.options.img_width / this.img_object.orig_width()*100;
        }
        else {
            new_zoom = this.options.img_height / this.img_object.orig_height()*100;
        }
...
//		console.log(x); //left
		...
		var left_element = $(this.options.left_element);
		var top_element = $(this.options.top_element);
		var rotate_element = $(this.options.rotate_element);

		if(this.options.left != 'auto') { x = this.options.left; }
		if(this.options.top != 'auto') { y = this.options.top; }
		top_element.val(y);
		left_element.val(x);

		var setted;
		if(setted != 1) {
			if(this.options.rotate != 'auto') { 
				var rotate = this.options.rotate;
				var img = this.img_object;
				this.img_object.angle(rotate)
				// console.log(this.img_object);
				rotate_element.val(rotate);				
			}
			var setted = 1;
		}
...
//		console.log('Old Y: '+old_y);
	...
	/*	if(this.current_load == "load")
        {	
//			var new_width = util.scaleValue(this.img_object.orig_width(), new_zoom);
//   		var new_height = util.scaleValue(this.img_object.orig_height(), new_zoom);
            var new_width = this.options.img_width;
   			var new_height = this.options.img_height;
			this.current_load = "";
			console.log('LOAD TRUE '+this.options.width);
        }
        else { */
			var new_width = util.scaleValue(this.img_object.orig_width(), new_zoom);
   			var new_height = util.scaleValue(this.img_object.orig_height(), new_zoom);
...
/******************* NASTAVENIE WIDTH A HEIGHT ****************************************/

//		if(this.options.img_width != 'auto') { new_width = this.options.img_width; }
//		if(this.options.img_height != 'auto') { new_height = this.options.img_height; }
...
//		console.log(new_height);
		...
		var width_element =  $(this.options.width_element);
		var height_element =  $(this.options.height_element);
		width_element.val(new_width);
		height_element.val(new_height);

        var coords = this._correctCoords( new_x, new_y ),
            self = this;

/*		console.log("-------------");
		console.log('New X corr: '+new_x);
		console.log('New Y corr: '+new_y); 
*/
		// TOP JE Y | LEFT je X
...
//		console.log('closest_rate '+closest_rate);
//		console.log('find_closest_zoom_rate '+this.find_closest_zoom_rate(this.current_zoom));
        var next_rate = closest_rate + delta;
        var next_zoom = this.options.zoom_base * Math.pow(this.options.zoom_delta, next_rate)
		
//		console.log('delta   : '+delta);
//		console.log('zoom dlt:'+this.options.zoom_delta);
//		console.log('nxt zoom: '+next_zoom);
//		console.log('cur zoom: '+this.current_zoom);
		...
		else {next_zoom *= this.options.zoom_delta;}
		
//		console.log('---------------');
//		console.log('nxt zoom mod: '+next_zoom);
...
//		console.log(deg);
		...
		var rotate_element =  $(this.options.rotate_element);
		rotate_element.val(deg);
		
		var isRotated =  $(this.options.isRotated_element);
//		console.log(this.options.isRotated_element);
		isRotated.val(true);
    ...
    **/
    _mouseStart: function( e )
    {
		...
		this.options.img_width = 'auto';
		this.options.img_height = 'auto';
		this.options.rotate = 'auto';
...
(function() {
    ...
    this.setImageProps = function(disp_w, disp_h, x, y, skip_animation, complete) {
        ...
        var h = this._swapDimensions ? disp_w : disp_h;
				...
				//console.log(this._swapDimensions);
				x = parseInt(x);
        var params = {
            width: w,
            height: h,
            ...
            left: x + (this._swapDimensions ? this.display_diff() / 2 : 0) + "px" 
				...
				console.log(this._swapDimensions);
				console.log('X: '+x);
				console.log("Scitam s X: "+this.display_diff() / 2);
				console.log("--------------");
				console.log(x + (this.display_diff() / 2));
				console.log("*******"); */
		/*
		var left_element = this.options.left_element;
		var top_element = this.options.top_element;
